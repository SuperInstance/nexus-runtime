"""
NEXUS i.MX RT1060 Hardware Configuration Module

NXP i.MX RT1060 — Cortex-M7 @ 600 MHz crossover MCU with 1 MB SRAM.
Designed for high-performance real-time marine sensor processing, motor
control, and edge AI inference in underwater robotics.

Key features:
  - Cortex-M7 @ 600 MHz (1328 DMIPS)
  - 1 MB tightly coupled SRAM (ITCM + DTCM + OCRAM)
  - FlexSPI for external flash (supports NOR, NAND, HyperFlash)
  - 2x FlexIO for custom protocols
  - 3x ADC, 2x DAC
  - 8x UART, 4x SPI, 4x I2C
  - 2x CAN-FD
  - 2x Enet (10/100 Mbps)
  - 2x USB OTG
  - 2x PWM (motor control submodule with 4 FLEs each)
  - LPUART, LPI2C, LPSPI low-power peripherals
"""
...
SRAM_TOTAL = 1024 * 1024              # 1 MB (ITCM + DTCM + OCRAM)
FLASH_TOTAL = 0                        # No internal flash (external via FlexSPI)
ADC_RESOLUTION = 12
ADC_CHANNELS = 6
PWM_MODULES = 2
PWM_SUBMODULES = 8                     # 4 per module
UART_COUNT = 8
SPI_COUNT = 4
I2C_COUNT = 4
CAN_FD_COUNT = 2
...
class GPIOPin(IntEnum):
    """Representative GPIO pin identifiers for i.MX RT1060 EVK."""
    GPIO_AD_B0_00 = 0
    GPIO_AD_B0_01 = 1
    GPIO_AD_B0_02 = 2
    GPIO_AD_B0_03 = 3
...
class ClockConfig:
    ...
    flexspi_freq_hz: int = 133_000_000

    def validate(self) -> List[str]:
        errors: List[str] = []
        if self.arm_freq_hz > CPU_FREQ_MAX:
            errors.append(f"ARM freq {self.arm_freq_hz} exceeds max {CPU_FREQ_MAX}.")
        if self.ahb_freq_hz > 150_000_000:
            errors.append("AHB freq cannot exceed 150 MHz.")
        return errors
...
class IMXRT1060Config:
    """
    Complete hardware configuration for the NXP i.MX RT1060.

    High-performance Cortex-M7 @ 600 MHz with 1 MB SRAM, designed
    for real-time marine sensor processing and motor control.
    """
    ...
    def _configure_default_pins(self):
        for func, pin in self.DEFAULT_PINS.items():
            self._pin_map[func] = pin
    ...
    def configure_marine_controller(self):
        """Set up SRAM for marine controller application."""
        self.memory.add_region("dtcm", 0x20000000, 256 * 1024, "ram", "Data TCM")
        self.memory.add_region("itcm", 0x00000000, 128 * 1024, "ram", "Instruction TCM")
        self.memory.add_region("ocram", 0x20200000, 512 * 1024, "ram", "On-chip RAM")
        self.memory.add_region("adc_dma", 0x20280000, 32 * 1024, "ram", "ADC DMA buffer")
        self.memory.add_region("eth_buf", 0x20288000, 64 * 1024, "ram", "Ethernet buffers")
        self.memory.add_region("can_buf", 0x20298000, 16 * 1024, "ram", "CAN-FD buffers")
